...
public class FilesResponse {
	 ...
	 */
	public FilesResponse(CloseableHttpResponse response, HttpUriRequest method) {
		this.method = method;
		this.response = response;

		if (logger.isDebugEnabled()) {
			logger.debug("Request Method: " + response);
			logger.debug("Request Path: " + method.getURI());
			logger.debug("Status Line: " + getStatusLine());
			Header[] reqHeaders = response.getAllHeaders();
			for (Header rH : reqHeaders)
				logger.debug(rH.getElements().toString());
